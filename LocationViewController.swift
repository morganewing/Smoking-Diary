...
class LocationViewController: UITableViewController, UITextFieldDelegate {
    ...
    @IBAction func addButton(_ sender: Any) {
        ...
        let newItemIndexPath = IndexPath(row: (locations.count + newLocation)-1, section: 0)
    ...
    @IBAction func deleteButton(_ sender: UIButton) {
        self.tableView.beginUpdates()
        let newItemIndexPath = IndexPath(row: (locations.count + new)-1, section: 0)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (indexPath.row < (locations.count + new)) {
